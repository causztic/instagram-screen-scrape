  ...
  InstagramPosts.prototype._read = function() {
    ...
    return getPosts(this.username, this._minPostId).on('error', (function(_this) {
      ...
      return function(rawPost) {
        ...
        switch (post.type) {
          case 'carousel':
            for (media of rawPost.carousel_media) {
              switch (media.type) {
                case 'image':
                  post.media.push(media.images['standard_resolution'].url);
                  break
                case 'video':
                  post.media.push(media.videos['standard_resolution'].url);
                  break
              }
            }
            break;
          case 'image':
            post.media.push(rawPost.images['standard_resolution'].url);
            break;
          case 'video':
            post.media.push(rawPost.videos['standard_resolution'].url);
            break;
          default:
