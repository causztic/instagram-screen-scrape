...
jsonRequest = function(jsonSelector, options) {
  ...
  options.gzip = false; //gzip doesn't work
  request(options).on('response', function(response){
    ...
    if (response && response.statusCode === 200) {
      ...
      return outStream.emit('error', "Instagram returned status: " + response.statusCode);
    }
  }).on('error', function(error){
    return outStream.emit('error', "Instagram returned: " + error);
  })
  return outStream;
};
